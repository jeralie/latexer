\question Let $G=(V, E)$ be an undirected graph. Match the term with the definition. \newline
\noindent\fbox{\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
\vspace{3mm}
\hspace{.5 cm} Walk \hspace{3.5 cm} Cycle  \hspace{3.5 cm} Tour \hspace{3.5 cm} Path\hspace{.5 cm}
\vspace{.9mm}
\end{minipage}}

\fbox{\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
 \line(1,0){150} Walk that starts and ends at the same node \\ \vspace{3mm}
 \line(1,0){150} Sequence of edges. \\ \vspace{3mm}
 \line(1,0){150} Sequences of edges with possibly repeated vertex or edge. \\ \vspace{3mm}
 \line(1,0){150} Sequence of edges that starts and ends on the same vertex and 
does not repeat vertices (except the first and last) 
\end{minipage}}

\begin{solution}
\fbox{\begin{minipage}{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}
 \center{\underline{tour}} Walk that starts and ends at the same node \\ \vspace{3mm}
 \center{\underline{path}} Sequence of edges. \\ \vspace{3mm}
 \center{\underline{walk}} Sequences of edges with possibly repeated vertex or edge.\\ \vspace{3mm}
 \center{\underline{cycle}} Sequence of edges that starts and ends on the same vertex and 
does not repeat vertices (except the first and last)
\end{minipage}}
\end{solution}


\question  What is a tournament?
\begin{solution}[1 in]
A directed graph for which any pair of vertices, $u, v$ either have an edge $u \rightarrow v$ or $v \rightarrow u$. It can be used to represent a tournament in which every pair of player play at least once.
\end{solution}

\question  What is a simple path?
\begin{solution}[1 in]
Sequence of edges where the vertices are distinct
\end{solution}